...
def run_orbit(client_id: str, config: str | None = Query(default=None)) -> dict:
    ...
    if project_dir:
        result = run_orbit_simulation(library=project_dir, config=(config or "base.yaml"), post_finalize_cb=_post_finalize_cb)
    else:
        result = run_orbit_simulation(config=(config or "base.yaml"), post_finalize_cb=_post_finalize_cb)
...
def trigger_orbit(client_id: str, config: str | None = Query(default=None)) -> dict:
    """Trigger an async ORBIT simulation and return a task_id to poll."""
    if not client_manager.get_client_project_dir(client_id):
        raise HTTPException(status_code=404, detail="Unknown client_id")
    project_dir = client_manager.get_client_project_dir(client_id)
    task_id = start_orbit_simulation_task(client_id, project_dir, config=config)
